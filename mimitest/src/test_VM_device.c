...
{
    int err = 0;
    device_t *device = New_device(0, NULL);
    device_t *zigbee = New_device_zigbee(0, NULL);
    device_t *voltage = New_device_voltage(0, NULL);
    device_t *light = New_device_light(0, NULL);

    args_t *args_in = New_args(0, NULL);
    args_t *args_in_zigbee = New_args(0, NULL);
    args_t *args_in_voltage = New_args(0, NULL);
    args_t *args_out = New_args(0, NULL);
    args_t *args_out_voltage = New_args(0, NULL);
    ...
    float floatOutput = 0;
    args_t *args_in_float = New_args(0, NULL);
    args_in_float->put_float(args_in_float, floatInput);
    ...
    char *arg_out1 = NULL, *arg_out2 = NULL;
    args_in->put(args_in, "arg1");
    args_in->put(args_in, "arg2");
    args_in_zigbee->put(args_in_zigbee, "test print to zigbee\r\n");
    ...
    args_out->get(args_out, 0, &arg_out1);
    args_out->get(args_out, 1, &arg_out2);

    if (0 != strcmp(arg_out1, "arg1"))
    {
        err = 2;
        goto exit;
    }
    if (0 != strcmp(arg_out2, "arg2"))
    {
        err = 3;
        goto exit;
    }

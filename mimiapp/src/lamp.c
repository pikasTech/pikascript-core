...
static void refresh_periodic_observer(gui_t *gui)
{
    ...
    lamp_t *lamp = (lamp_t *)(panel->context);
    {
        ...
        gui->showLine(gui, 3, 1, buff);
    }
    {
        char buff[32];
        sprintf(buff, "V: %1.4f V", lamp->voltage);
        gui->showLine(gui, 4, 1, buff);
    }
    {
        char buff[32];
        sprintf(buff, "I: %1.4f mA", lamp->current);
        gui->showLine(gui, 5, 1, buff);
    }
}

static void closeLight_callBack(gui_t *gui)
{
...
static void openLight_callBack(gui_t *gui)
{
    //rewrite in user code
    panel_lamp_t *panel = gui->context;
    lamp_t *lamp = panel->context;
...
static void enableInductor_callBack(gui_t *gui)
{
    panel_lamp_t *panel = gui->context;
    lamp_t *lamp = panel->context;
...
static void disableInductor_callBack(gui_t *gui)
{
    panel_lamp_t *panel = gui->context;
    lamp_t *lamp = panel->context;
...
static void gui_setTime_refresh(gui_t *gui)
{
    panel_lamp_t *panel = gui->context;
    lamp_t *lamp = panel->context;
...
static void timerSet_ok(gui_t *gui)
{
...
static void timer_start(gui_t *gui)
{
    panel_lamp_t *panel = gui->context;
    lamp_t *lamp = panel->context;
    lamp->light->open(lamp->light);
...
lamp_t *Class_lamp_init(void)
{
    ...
    lamp->panel->gui_timing->option_callBack[1] = timer_start;
    ...
    lamp->zigbee = New_device_zigbee(0, NULL);
    lamp->vol = New_device_voltage(0, NULL);
    lamp->light_sensor = New_device_light(0, NULL);
    lamp->blueTeeth = New_device_blueTeeth(0, NULL);
    return lamp;
}